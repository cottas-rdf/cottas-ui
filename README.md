...
- **Exploration** of metadata, statistics, and triple samples.
...
- **Merge** of multiple COTTAS files into a single graph.
...
- `merge` and `diff` operations materialize the result with the user-selected index.
- Metadata and triple samples are cached with `st.cache_data` to avoid recomputation when the file has not changed.
...
The suite includes unit tests for the validation, file management, statistics, and pycottas bridge layers, as well as integration tests for the complete compression, SPARQL query, difference, and merge flows.
...
The `Dockerfile` starts from `python:3.11-slim`, installs the dependencies pinned in `requirements.txt`, exposes port `8501`, and launches Streamlit in headless mode. The `docker-compose.yml` adds a volume for temporary files (`/tmp/cottas_app`) and a healthcheck against `/_stcore/health`.
...
│   ├── stats.py              # Metrics and visualizations
...
| Explore    | `.cottas`                         | metadata + statistics           |
